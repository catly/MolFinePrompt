...
class GNN_finetune(torch.nn.Module):
    ...
    def group_node_rep(self, node_rep, batch_size, num_part):
        super_group = []
        count = 0
    ...
    def forward(self, *argv):
        if len(argv) == 4:
            ...
            data = argv[0]
            x, edge_index, edge_attr, num_part = data.x, data.edge_index, data.edge_attr, data.num_part
        else:
            raise ValueError("unmatched number of arguments.")

        node_rep = self.gnn(x, edge_index, edge_attr)

        super_node_rep = self.group_node_rep(node_rep, len(num_part), num_part)
        super_node_rep = torch.stack(super_node_rep, 0)
